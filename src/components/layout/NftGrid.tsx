...
import type { NftCatalogItem } from '@/app/_data/catalog-data';
import { Card } from '@/components/nft/NftCard';
import { Button } from '@/components/ui/Button';
import { cn } from '@/lib/utils';

type NftGridProps = {
  items: NftCatalogItem[];
  addedIds: Set<string>;
  onBuy: (item: NftCatalogItem, sourceEl: HTMLElement | null) => void;
    ...
    >
      {items.map((item, index) => {
        ...
        return (
          <div
            key={item.id}
            className={cn('w-[345px]', shouldAnimate && 'animate-card-enter')}
          >
            <Card>
